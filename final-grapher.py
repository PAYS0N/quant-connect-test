...
class OptionsDataGraphAlgorithm(QCAlgorithm):
    def initialize(self):
        self.set_start_date(2025, 4, 1)
        self.set_end_date(2025, 5, 14)
        self.set_cash(100000)
        self.target_expiry = datetime(2025, 5, 16).date()
        self.target_strikes = [20]
        option = self.add_option("INTC")
    ...
    def on_data(self, slice):
        ...
        calls = [c for c in chain if c.right == OptionRight.CALL
                 and abs((c.expiry.date() - self.target_expiry).days) < 7
